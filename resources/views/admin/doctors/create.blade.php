        ...
        <form method="POST" action="{{ route('admin.doctors.store') }}">
                        ...
                        <table class="schedule-table">
                            ...
                            <tbody>
                                @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                                    @php 
                                        $isChecked = is_array(old('available_days')) && in_array($day, old('available_days'));
                                    @endphp
                                        ...
                                        <td class="time-cell">
                                            <div class="input-wrapper">
                                                ...
                                                <input type="time" name="schedules[{{ $day }}][start_time]" 
                                                    value="{{ old('schedules.'.$day.'.start_time') }}"
                                        ...
                                        <td class="time-cell">
                                            <div class="input-wrapper">
                                                ...
                                                <input type="time" name="schedules[{{ $day }}][end_time]" 
                                                    value="{{ old('schedules.'.$day.'.end_time') }}"
                                                    class="schedule-input" {{ $isChecked ? '' : 'disabled' }}>
                                            </div>
                                        </td>
                                        <td class="time-cell">
                                            <select name="schedules[{{ $day }}][slot_duration]" class="duration-select" {{ $isChecked ? '' : 'disabled' }}>
                                                @foreach([10, 15, 20, 30, 45, 60] as $dur)
                                                    <option value="{{ $dur }}" {{ old('schedules.'.$day.'.slot_duration') == $dur ? 'selected' : ($dur == 15 ? 'selected' : '') }}>{{ $dur }} min</option>
                                                @endforeach
                                            </select>
                                        </td>
                                    </tr>
            ...
            <div class="actions">
                <button type="submit" class="btn-primary">
                    <i class="fas fa-check"></i> Register Doctor
                </button>
