...
@section('content')
    ...
    <div class="card" style="max-width: 600px; margin: 0 auto;">
        <form action="{{ route('pharmacist.medicines.update', $medicine->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 8px; font-weight: 600;">Medicine Name</label>
                <input type="text" name="name" class="search-input" style="position: static;" value="{{ $medicine->name }}" required>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Stock Quantity</label>
                    <input type="number" name="quantity" class="search-input" style="position: static;" min="0" value="{{ $medicine->quantity }}" required>
                </div>
                <div>
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Price (Per Unit)</label>
                    <input type="number" name="price" class="search-input" style="position: static;" min="0" step="0.01" value="{{ $medicine->price }}" required>
                </div>
            </div>
